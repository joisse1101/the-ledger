...
It listens on port 10080 by default (override with `$env:GATEWAY_PORT` before running the
...
`http://<this machine's LAN address>:10080/?token=<token>` (the token is the contents of
