...
- **BREAKING**: `streamlit run app.py` no longer exists. The app starts with two commands: `python api/server.py` (the API) and, in `web/`, `npm run preview -- --host` (the built frontend, after a one-time `npm run build`). `app.py`, `views/`, `.streamlit/`, and the `streamlit`/`pandas` runtime dependencies are removed. Node.js is now needed to build and serve the front end.
