...
- [Two processes to start instead of one] → Matches the dev-mode workflow that already existed (`python server.py` + `npm run dev`); the README documents both commands for LAN/production the same way it already does for dev, and `--frontend-port` keeps the API's printed link in sync with wherever the frontend actually runs.
