...
**Goals:**
- Two processes — an API-only backend and a statically-served frontend, each on its own port and started with its own command (the same shape dev mode already uses; there's no separate "dev" vs. "production" serving story on the frontend side). Python data layer unchanged apart from `db_path()` and SQLite pragmas.
...
**2. Two servers: FastAPI serves only `/api`; the built frontend is served by Vite's own `vite preview --host`.** `api/server.py` no longer mounts `web/dist` or falls back to `index.html` for anything — it is a pure JSON API on its own port (default 8501). The frontend is served by running `vite preview --host` (in `web/`) against the build output, on its own port (Vite's own default, 4173, overridable); `--host` binds it to every interface the same way `--lan` does for the API. Two processes, two ports, one command each — matching the dev-mode shape (`npm run dev` proxying `/api` to 8501) that already existed, so there's no longer a separate "dev" vs. "production" serving model for the frontend. `vite preview` has no auth logic of its own: the static shell it serves (HTML/JS/CSS, no data) is reachable by anyone who can reach that port; only the API calls the shell makes are token-gated (Decision 7). Responses from the API still go through gzip middleware. CORS is added to the API (Decision 7) so the frontend's origin can call it.
*Alternatives:* keep serving `web/dist` from FastAPI and have the frontend call it same-origin (the original design here) — rejected per the user: `api/server.py` should be the API only, and `web/` should be the one and only frontend, not something FastAPI also half-serves. Have `vite preview` (or a thin reverse proxy in front of it) proxy `/api/*` to the backend, so the browser only ever talks to one origin and cookies/no-CORS could stay as they were — a lighter-weight option that avoids all of the CORS/bearer-token work in Decision 7; not taken now in favor of the cleaner frontend/backend split the user asked for, but worth revisiting if that surface proves troublesome. Gate the static shell behind the token too, via a check in `vite preview`'s middleware hook (rejected: duplicates auth logic in a second language for a shell that carries no data). Commit or publish a prebuilt `dist/` (rejected, decided: build output stays out of git and releases; anyone running the app builds it once with `npm ci && npm run build`).
...
**4. Server does the aggregation, filtering, sorting, and paging.** Overview stats are computed server-side from the same transcript rows (`overview_stats.py`, the pure functions lifted out of `streamlit_app/views/overview.py` with the range table, calendar-date bucketing, top-7 + Other, and hourly trim intact). The All list is filtered/sorted/paged in `transcript_query.py`, a plain-Python port of `_filter_transcripts_dataframe`/`_sort_transcripts_dataframe` (literal, case-insensitive substring match; stable sort; missing values last in both directions). This keeps a phone's payload to one page (default 50) rather than the full history with 600-character snippets, and keeps the semantics in tested Python. Pandas is dropped if nothing else imports it once `streamlit_app/views/` is gone.
...
**7. Auth: a bearer token the frontend holds, not a cookie; CORS opened only to the frontend's own origins.**
- *Who is local:* unchanged from the original design — `request.client.host` in loopback (`127.0.0.1`, `::1`, `::ffff:127.0.0.1`) **and** none of `Forwarded`, `X-Forwarded-For`, `X-Real-IP` present. This is evaluated against whoever calls the **API** (the frontend's JS, running in a browser on some device), regardless of which device served the page it's running in. Under `--lan`, opening the *frontend* via the machine's own LAN IP still means its API calls arrive non-loopback and need the token; opening the frontend via `localhost` means they don't. That's acceptable and documented.
- *Token source:* unchanged — `LEDGER_TOKEN` env var, else `.ledger/token` (created on first `--lan` run with `secrets.token_urlsafe(32)`, 256 bits). Compared with `hmac.compare_digest`.
- *Presenting it:* every non-local API request needs `Authorization: Bearer <token>`. The API has no `?token=` handling, no redirect, and sets no cookie — it only ever reads the header. The **frontend** owns the sign-in UX instead: on load it checks its own URL for `?token=`, and if present, stores it in `localStorage` and strips it from the address bar (`history.replaceState`, no reload) so it isn't left in history; every API call reads the token from `localStorage` and attaches the header. A wrong or missing token gets a `401` from the API with a short message; the frontend turns that into an "open the printed link" state.
- *The static shell is not gated.* `vite preview` serves `index.html`/JS/CSS to anyone who can reach its port, LAN or not — there's no data in that shell, only the API calls it makes are gated. This is narrower than the original design's "gate everything, static files included," and the `network-access` spec now says so explicitly.
- *CORS is enabled, allow-listed to the frontend's own origins only — never a wildcard.* The API computes the same address set it prints in the startup banner (Decision 12): `http://localhost:<frontend-port>`, `http://127.0.0.1:<frontend-port>`, and `http://<ip>:<frontend-port>` for each discovered LAN IP; only a request whose `Origin` matches one of those gets `Access-Control-Allow-Origin` reflected back. `allow_credentials` is off (nothing is credentialed now that there's no cookie); allowed headers are `Authorization`, `X-Requested-With`, `Content-Type`; allowed methods are `GET`, `POST`, `DELETE`. This matters beyond tidiness: the API still treats loopback callers as needing no token, and if CORS reflected *any* origin, a malicious page open in another tab on the same machine could `fetch("http://127.0.0.1:8501/api/live")` and, being loopback, get data back with no token — and with open CORS, read it. Allow-listing only the known frontend origins closes that: an unlisted origin's JS still can't read the response, even though the loopback request itself completes server-side (the same blind-GET exposure a same-origin, no-CORS design has today).
- *Cross-site protection for state-changing requests:* every non-GET request still must carry `X-Requested-With: ledger`, which the frontend adds. An unlisted cross-site `Origin` can't get that header past a CORS preflight, so a blind cross-site `POST`/`DELETE` is refused before it's ever sent, regardless of the CORS allow-list — defense in depth alongside the fact that a third-party page can't read the frontend's `localStorage` to forge the `Authorization` header either.
- *DNS rebinding:* unchanged — for loopback clients, `Host` must be `localhost`, `127.0.0.1`, or `[::1]` (any port), else `403`. This still matters independently of CORS: it's what stops a DNS name that resolves to `127.0.0.1` from being classified as a local caller in the first place, before CORS even enters the picture.
- *Logs:* uvicorn access logging stays off. There's no `?token=…` reaching the API by design now (the frontend strips it before ever calling the API), but the `Authorization` header shouldn't be logged either.
*Alternatives:* HTTP Basic auth (rejected: browsers re-prompt and phones can't easily use a URL/QR flow); per-device tokens (rejected as scope; the shared token is what the user chose); read-only mode for remote clients (the user chose token over this); keep the cookie and use credentialed CORS restricted to the frontend's exact origin (rejected: still needs the same origin allow-list below, but adds `SameSite`/`Secure`/credentialed-CORS interactions for no benefit over a header the frontend already controls itself).
...
**11. Theme is a client concern.** `data-theme` on `<html>`, initialized from `localStorage` (guarded with try/catch) else `prefers-color-scheme`, toggled by the nav control. The categorical palette and neutral tokens from `streamlit_app/views/overview.py` become CSS variables (light and dark sets, same slot order) that both the CSS and the chart specs read, so the palette keeps its documented fixed-order property. `.streamlit/theme_pref.json` and the process-wide `st_config.set_option` hack disappear.

**12. Entry point and LAN startup — the API prints a link to the frontend, not to itself.** `api/server.py` is both the API's ASGI app and its CLI: `python api/server.py [--lan] [--host H] [--port P] [--frontend-port FP]` (env `LEDGER_HOST`/`LEDGER_PORT`/`LEDGER_FRONTEND_PORT` as defaults). Default host `127.0.0.1`; `--lan` means `0.0.0.0`. `--frontend-port` defaults to Vite's own preview default (4173) and is used only to build the printed/QR URL and the CORS allow-list (Decision 7) — the API never talks to that port itself. On start it prints the local frontend URL (`http://localhost:<frontend-port>/`) and a reminder that the frontend must be running separately for that address to answer; with `--lan` it also discovers the machine's IPv4 addresses (hostname resolution plus the UDP-connect trick to find the primary interface, without sending packets; drops loopback and 169.254.x.x), prints `http://<ip>:<frontend-port>/?token=<token>` for each, an ASCII QR of the first via `qrcode`, and the plain-HTTP warning. The frontend is started separately (`cd web && npm run preview -- --host`, mirroring the existing dev-mode `npm run dev`); nothing here spawns it. The first `--lan`/`--host` run on Windows will pop the firewall prompt (potentially for both processes, if their ports differ); the README says to allow Private networks and gives the `netsh` alternative.
...
**14. Testing.**
- Python: pytest as today, plus API tests with FastAPI's `TestClient` (needs `httpx`). Remote clients are simulated with `TestClient(app, client=("192.168.1.50", 5000))`; forwarded-header and Host cases are plain header tests. Ported: `test_overview.py` → `overview_stats`; the filter/sort cases of `test_views.py` → `transcript_query`; new: bearer-token auth matrix (no more cookie/redirect cases), CORS origin allow-list (matching origin reflected, unrelated origin gets no CORS header, `allow_credentials` off), token persistence, id validation, delete-live-409, coalescing (recompute count). Static-serving/missing-`dist` behavior moves out of the API's tests entirely, since `api/server.py` no longer serves the frontend. `tests/test_app.py` goes with `_auto_refresh_due`.
...
- [The static app shell is publicly reachable by anyone on the LAN once `vite preview --host` is running, even without a token] → Accepted per Decisions 2/7: the shell carries no data, and every API call remains gated; the `network-access` spec says this explicitly rather than leaving it implicit.
- [A wildcard CORS origin would let any other locally-open tab read loopback-exempt API responses] → Mitigated by allow-listing exactly the frontend's known origins (Decision 7), never `*`; a test asserts an unrelated `Origin` gets no CORS header.
- [Two processes to start instead of one] → Matches the dev-mode workflow that already existed (`python api/server.py` + `npm run dev`); the README documents both commands for LAN/production the same way it already does for dev, and `--frontend-port` keeps the API's printed link in sync with wherever the frontend actually runs.
...
3. In one final group of tasks: move the DB path to `.ledger/`, delete `streamlit_app/app.py`, `streamlit_app/views/`, `.streamlit/`, Streamlit-only tests and dependencies, update `.gitignore`, `CLAUDE.md`, and `README.md`.
