...
- [x] 4.2 Re-run `hooks/Install-ClaudeHooks.ps1` to refresh the installed copy. Verify `hooks/scripts/Send-ClaudeToast.ps1` and `~/.claude/hooks/Send-ClaudeToast.ps1` are identical (`diff` reports no differences), and `openspec validate toast-token-usage --strict` still reports valid.
